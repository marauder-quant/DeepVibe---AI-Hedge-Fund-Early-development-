...
def read_output(pipe, script_name):
    """Read lines from a subprocess pipe and add to shared log."""
    try:
        for line in iter(pipe.readline, ''):
            if not keep_running:
                break
                
            # Only log if it's a trade execution line or critical error
            # This drastically reduces output noise
            line_str = line.strip()
        ...
        pipe.close()
         ...
         with log_lock:
            log_lines.append(f"[ERROR {script_name}] Pipe read error: {e}")
...
def start_bots(script_dir):
    ...
    for script in BOT_SCRIPTS:
        ...
        try:
            ...
            Thread(target=read_output, args=(process.stdout, script), daemon=True).start()
            Thread(target=read_output, args=(process.stderr, f"{script}(err)"), daemon=True).start()
