...
# Volatility ETF ticker symbol
VOLATILITY_TICKER = "SH"
...
def get_volatility_allocation():
    """
    Get the volatility allocation based on the A quadrant hedge setting
    Returns the allocation percentage (0.0-1.0)
    """
    # Calculate the volatility allocation from config
    volatility_allocation = config.QUADRANT_A_HEDGE_ALLOCATIONS['Volatility']
    print(f"Volatility allocation: {volatility_allocation:.2%} of portfolio")
    return volatility_allocation
...
def calculate_buy_quantity(symbol, current_price):
    """Calculate number of shares to buy based on volatility allocation"""
    try:
        account = trading_client.get_account()
        portfolio_value = float(account.portfolio_value)
        
        # Calculate the target value for volatility based on config
        volatility_allocation = get_volatility_allocation()
        target_trade_value = portfolio_value * volatility_allocation
        
        print(f"Portfolio value: ${portfolio_value:.2f}")
        print(f"Volatility allocation: {volatility_allocation:.2%}")
        print(f"Target trade value: ${target_trade_value:.2f}")
...
def run_volatility_strategy(last_bar_time=None, force_check=False):
    """Run the SMA crossover strategy for SPXS, only trading during Quadrant A"""
    symbol = VOLATILITY_TICKER
    ...
    if not quadrant_a:
        print(f"Current quadrant is not A. Volatility bot is in sleep mode, monitoring only.")
...
if __name__ == "__main__":
    ...
    # Get quadrant information and volatility allocation
    quadrant_info = get_current_quadrant()
    volatility_allocation = get_volatility_allocation()
    
    print(f"=== VOLATILITY TRADING BOT ===")
    print(f"Current economic quadrant: {quadrant_info['quadrant']}")
    print(f"Notes: {quadrant_info['notes']}")
    print(f"Volatility allocation: {volatility_allocation:.2%} of portfolio")
    print(f"Volatility trading active: {'Yes' if quadrant_info['quadrant'] == 'A' else 'No - Sleep mode'}")
    ...
    # Variable to store the latest bar time
    last_bar_time = None
    
    try:
        # Run strategy immediately on startup and force a check
        last_bar_time = run_volatility_strategy(force_check=True)
        ...
        while True:
            ...
            # After waiting, run the strategy
            last_bar_time = run_volatility_strategy(last_bar_time)
        ...
        print("Volatility bot stopped. Final position:")
        volatility_position = get_position_quantity(VOLATILITY_TICKER)
        if volatility_position > 0:
            position = next((p for p in trading_client.get_all_positions() if p.symbol == VOLATILITY_TICKER), None)
            if position:
            ...
            print(f"No position in {VOLATILITY_TICKER}") 