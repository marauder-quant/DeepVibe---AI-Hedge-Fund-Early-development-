...
def get_gold_allocation():
    """
    Get the gold allocation based on the A quadrant hedge setting
    Returns the allocation percentage (0.0-1.0)
    """
    # Calculate the gold allocation properly - fixing the formula from config
    # The correct formula should be: (1 - QUADRANT_ALLOCATIONS['A']) * 0.5
    # Instead of using config.QUADRANT_A_HEDGE_ALLOCATIONS['Gold'] which has a parenthesis issue
    gold_allocation = config.QUADRANT_A_HEDGE_ALLOCATIONS['Gold']
    print(f"Gold allocation: {gold_allocation:.2%} of portfolio")
    return gold_allocation
...
def calculate_buy_quantity(symbol, current_price):
    """Calculate number of shares to buy based on gold allocation"""
    try:
        account = trading_client.get_account()
        portfolio_value = float(account.portfolio_value)
        
        # Calculate the target value for gold based on config
        gold_allocation = get_gold_allocation()
        target_trade_value = portfolio_value * gold_allocation
        
        print(f"Portfolio value: ${portfolio_value:.2f}")
        print(f"Gold allocation: {gold_allocation:.2%}")
...
def run_gold_strategy(last_bar_time=None, force_check=False):
    """Run the SMA crossover strategy for GLD, only trading during Quadrant A"""
    symbol = GOLD_TICKER
    ...
    if not quadrant_a:
        print(f"Current quadrant is not A. Gold bot is in sleep mode, monitoring only.")
...
if __name__ == "__main__":
    ...
    # Get quadrant information and gold allocation
    quadrant_info = get_current_quadrant()
    gold_allocation = get_gold_allocation()
    
    print(f"=== GOLD TRADING BOT ===")
    print(f"Current economic quadrant: {quadrant_info['quadrant']}")
    print(f"Notes: {quadrant_info['notes']}")
    print(f"Gold allocation: {gold_allocation:.2%} of portfolio")
    print(f"Gold trading active: {'Yes' if quadrant_info['quadrant'] == 'A' else 'No - Sleep mode'}")
    ...
    try:
        ...
        last_bar_time = run_gold_strategy(force_check=True)
        ...
        while True:
            ...
            last_bar_time = run_gold_strategy(last_bar_time)
        ...
        print("Gold bot stopped. Final position:")
        gold_position = get_position_quantity(GOLD_TICKER)
        if gold_position > 0:
            position = next((p for p in trading_client.get_all_positions() if p.symbol == GOLD_TICKER), None)
            if position:
            ...
            print(f"No position in {GOLD_TICKER}") 