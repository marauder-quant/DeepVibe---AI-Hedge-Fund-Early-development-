...
def get_bid_ask_prices(symbol):
    """
    Get the current bid and ask prices for a symbol using Alpaca Market Data API.
    Returns (bid_price, ask_price) tuple.
    """
    try:
        ...
        if latest_quote and symbol in latest_quote:
            ...
            print(f"Retrieved Alpaca quote for {symbol}: bid=${bid_price:.2f} (size:{bid_size}), ask=${ask_price:.2f} (size:{ask_size})")
            return bid_price, ask_price
        else:
            print(f"No quote data found for {symbol} from Alpaca")
            
            # Fall back to getting current price from yfinance if Alpaca quote is unavailable
            ticker = yf.Ticker(symbol)
            current_price = ticker.info.get('regularMarketPrice', None) or ticker.info.get('currentPrice', None)
            
            if current_price and current_price > 0:
                # Create synthetic bid/ask with realistic spread based on price level
                if current_price < 10:
                    spread_factor = 0.005  # 0.5% spread for low-priced stocks
                elif current_price < 50:
                    spread_factor = 0.002  # 0.2% spread
                elif current_price < 200:
                    spread_factor = 0.001  # 0.1% spread
                else:
                    spread_factor = 0.0005  # 0.05% spread for high-priced stocks
                
                bid_price = current_price * (1 - spread_factor)
                ask_price = current_price * (1 + spread_factor)
                print(f"Using fallback synthetic bid/ask for {symbol}: bid=${bid_price:.2f}, ask=${ask_price:.2f}")
                return bid_price, ask_price
            
            print(f"Failed to get reliable price data for {symbol}")
            return None, None
            
    except Exception as e:
        print(f"Error getting Alpaca quote data for {symbol}: {e}")
        print("Falling back to yfinance for current price")
        
        try:
            # Fallback to yfinance for a current price
            ticker = yf.Ticker(symbol)
            ...
            if current_price and current_price > 0:
                ...
                print(f"Using fallback price for {symbol}: ${current_price:.2f} (synthetic bid/ask)")
                return bid_price, ask_price
        except Exception as fallback_error:
            print(f"Fallback to yfinance also failed for {symbol}: {fallback_error}")
...
def place_order(symbol, side, quantity, current_price):
    """Place limit orders for both market hours and after-hours trading."""
    ...
    try:
        if is_regular_market_hours():
            # During market hours - use limit orders based on bid/ask to operate as a maker
            bid_price, ask_price = get_bid_ask_prices(symbol)
            
            if bid_price is None or ask_price is None:
                print(f"Could not get bid/ask prices for {symbol}. Using current price as fallback.")
                # Fall back to using current price if bid/ask retrieval failed
                if side == OrderSide.BUY:
                    limit_price = current_price * 0.999  # Slight discount for buy
                else:
                    limit_price = current_price * 1.001  # Slight premium for sell
            else:
                # Use the actual bid/ask prices
                if side == OrderSide.BUY:
                    limit_price = bid_price  # Buy at the bid (as a maker)
                else:
                    limit_price = ask_price  # Sell at the ask (as a maker)
            ...
            print(f"Market hours - Using limit order for {side.name} at price ${limit_price:.2f} (acting as maker)")
        else:
            # After hours - use limit orders with aggressive pricing (unchanged behavior)
...
def run_strategy_for_all_symbols(last_bar_times=None, force_check=False):
    ...
    print(f"Using stocks selected based on economic quadrant analysis ({len(quadrant_symbols)}): {', '.join(quadrant_symbols)}")
    ...
    # print(f"Stocks: {', '.join(symbols)}") # Commented out old print
...
if __name__ == "__main__":
    ...
    print(f"IMPORTANT: Using limit orders during market hours - buying at the bid, selling at the ask (acting as maker)")
