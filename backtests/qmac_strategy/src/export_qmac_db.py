...
"""
QMAC Database Export Utility
Exports parameter data from the QMAC database to CSV format.
"""
...
from lib.database import get_parameters_from_db, DB_PATH

def export_qmac_db_to_csv(output_dir=None, symbol=None, timeframe=None, format_output=True):
    """
    Export QMAC database content to CSV files
    ...
    if df.empty:
        print(f"No data found in database at {DB_PATH}")
    ...
    if format_output:
        ...
        filename = f'qmac_{symbol_part}_{timeframe_part}_{timestamp}.csv'
    else:
        filename = 'qmac_parameters.csv'
...
def print_database_summary():
    """
    Print a summary of the QMAC database content
    """
    if not os.path.exists(DB_PATH):
        print(f"Database not found at {DB_PATH}")
        return
...
def main():
    ...
    """
    parser = argparse.ArgumentParser(description='QMAC Database Export Utility')
